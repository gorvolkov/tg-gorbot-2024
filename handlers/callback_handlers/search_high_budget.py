...
@bot.callback_query_handler(func=lambda call: (call.data == "high_budget"))
def ask_genre(call: CallbackQuery) -> None:
    """Хэндлер сценария поиска фильмов с высоким бюджетом (1 шаг сценария).
    Запрашивается жанр"""
...
@bot.callback_query_handler(
    state=SearchState.hb_genre, func=lambda call: (call.data.startswith("genre"))
)
def ask_count(call: CallbackQuery) -> None:
    """Хэндлер сценария поиска фильмов с высоким бюджетом (2 шаг сценария).
    ...
    bot.set_state(call.from_user.id, SearchState.hb_count)
