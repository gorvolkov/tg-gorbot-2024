...
from utils.input_checker import check_and_format_rating_input


@bot.callback_query_handler(func=lambda call: (call.data == "movies_by_rating"))
def ask_genre(call: CallbackQuery) -> None:
    """Хэндлер для запуска поиска фильмов по рейтингу (1 шаг сценария). Запрашивается жанр"""
...
@bot.callback_query_handler(
    state=SearchState.r_genre, func=lambda call: (call.data.startswith("genre"))
)
def ask_rating(call: CallbackQuery) -> None:
    """Хэндлер сценария поиска фильмов по рейтингу (2 шаг сценария). Запрашивается рейтинг"""

    bot.delete_message(call.from_user.id, call.message.id)
    query_genre: str = call.data.split("_")[1]

    with bot.retrieve_data(call.from_user.id) as data:
        data["genre"] = query_genre

    bot.send_message(
        call.from_user.id, "Введите рейтинг (число от 1 до 10 включительно):"
    )
    bot.set_state(call.from_user.id, SearchState.r_rating)


@bot.message_handler(state=SearchState.r_rating)
def ask_count(message: Message) -> None:
    """Хэндлер сценария поиска фильмов по рейтингу (3 шаг сценария).
    Выполняется проверка корректности введённого значения рейтинга.
    В случае успеха запрашивается кол-во результатов в выборке"""

    query_rating = check_and_format_rating_input(message.text)

    if not query_rating:
        bot.send_message(
            message.from_user.id,
            "Здесь может быть только число от 1 до 10 включительно.\n "
            "Пожалуйста, введите корректное значение!",
        )
    else:
        with bot.retrieve_data(message.from_user.id) as data:
            data["rating"] = query_rating

        bot.send_message(message.from_user.id, "Введите количество фильмов в выборке:")
        bot.set_state(message.from_user.id, SearchState.r_count)
...
def give_result(message: Message) -> None:
    ...
    if not message.text.isdigit():
        ...
        result = get_by_rating(
            genre=data["genre"], rating=data["rating"], count=data["count"]
        )
...
@bot.callback_query_handler(
    state=SearchState.r_count, func=lambda call: (call.data == "continue")
)
