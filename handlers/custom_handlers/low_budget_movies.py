...
def ask_genre(message: Message) -> None:
    ...
    bot.set_state(message.from_user.id, MovieSearchState.low_budget_genre, message.chat.id)


@bot.message_handler(state=MovieSearchState.low_budget_genre)
def ask_count(message: Message) -> None:
    if message.text.lower() not in GENRES_SET:
        ...
        bot.set_state(message.from_user.id, MovieSearchState.low_budget_count, message.chat.id)


@bot.message_handler(state=MovieSearchState.low_budget_count)
